...
test('serves only /updates/* on the updates domain', async () => {
  const feed = await worker.fetch(new Request('https://browserdaddy.significanthobbies.com/updates/appcast.xml'), env);
...
test('rejects non-read methods', async () => {
  const response = await worker.fetch(new Request('https://browserdaddy.significanthobbies.com/updates/appcast.xml', { method: 'POST' }), env);
